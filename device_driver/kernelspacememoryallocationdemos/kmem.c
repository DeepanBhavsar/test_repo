...
long ei_char_ioctl(struct file *fp, unsigned int ioctl_no, unsigned long ioctl_param)
{
	...
	switch (ioctl_no)
	{
		case IOCTL_SET_MSG:
			...
			copy_from_user((char *)s,(char *)ioctl_param,3);
			s[3]='\0';
			printk(KERN_INFO "ans : %s",s);
			break;
		case IOCTL_GET_MSG:
...
int ei_char_open(struct inode *node, struct file *fp)
{
	...
	kbuf = kmalloc(sizeof(int)*260,GFP_KERNEL);
	if (kbuf == NULL)
	{
		printk(KERN_ALERT "kmalloc buffer can not be allocated\n");
		return -1;
	}
		
	printk(KERN_INFO "Allocated kmalloc bytes are %d\n",ksize(kbuf));
        return 0;
}
